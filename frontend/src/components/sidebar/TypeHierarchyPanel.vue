...
async function refresh() {
  ...
  const position = model.getPositionAt(model.getOffsetAt({ lineNumber: 1, column: 1 }));
  // 实际应获取当前光标位置，这里简化处理
  const cursorPosition = editorStore.activeCursor || { lineNumber: 1, column: 1 };
...
function getLanguage(path: string): string {
  ...
  const map: Record<string, string> = {
    ts: 'typescript', js: 'javascript', go: 'go', py: 'python', java: 'java'
  };
  return map[ext || ''] || '';
}
</script>
